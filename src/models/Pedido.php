...
class Pedido {
    ...
    public function actualizarEstado($pedido_id, $nuevo_estado) {
        ...
        try {
            ...
            $stmt_estado->bindParam(':pedido_id', $pedido_id, PDO::PARAM_INT);
            $stmt_estado->execute();
            $pedido = $stmt_estado->fetch(PDO::FETCH_ASSOC);
            ...
            // Si el pedido pasa a "procesando" (en curso), descontamos el stock
            if ($nuevo_estado === 'procesando' && $estado_anterior !== 'procesando') {
                $this->descontarStock($pedido_id);
            }
            
            // Si el pedido se cancela y antes estaba en procesando, enviado o entregado, devolvemos el stock
            if ($nuevo_estado === 'cancelado' && in_array($estado_anterior, ['procesando', 'enviado', 'entregado'])) {
                $this->restaurarStock($pedido_id);
            }
     ...
     */
    private function descontarStock($pedido_id) {
        $sql = "SELECT producto_id, cantidad FROM pedidos_detalle WHERE pedido_id = :pedido_id";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':pedido_id', $pedido_id, PDO::PARAM_INT);
        $stmt->execute();
        ...
        foreach ($items as $item) {
            ...
            if (!$stmt_stock->execute()) {
                throw new Exception("Error al descontar stock del producto " . $item['producto_id']);
            }
        }
    }
    
    /**
     * Restaurar stock de los productos del pedido (en caso de cancelación)
     */
    private function restaurarStock($pedido_id) {
        $sql = "SELECT producto_id, cantidad FROM pedidos_detalle WHERE pedido_id = :pedido_id";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':pedido_id', $pedido_id, PDO::PARAM_INT);
        $stmt->execute();
        ...
        foreach ($items as $item) {
            ...
            if (!$stmt_stock->execute()) {
                throw new Exception("Error al restaurar stock del producto " . $item['producto_id']);
            }
        }
    }
